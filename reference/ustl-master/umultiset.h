...
public:
...
#if HAVE_CPP11
    inline explicit		multiset (multiset&& v)			: base_class (forward<multiset>(v)) {}
    inline			multiset (initlist_t v)			: base_class() { insert (v.begin(), v.end()); }
    inline multiset&		operator= (multiset&& v)		{ base_class::operator= (forward<multiset>(v)); return (*this); }
    inline iterator		insert (T&& v)				{ return (base_class::insert (upper_bound(v), forward<T>(v))); }
    ...
    inline void			swap (multiset&& v)			{ base_class::swap (forward<multiset>(v)); }
#endif
};
