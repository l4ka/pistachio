...
public:
  ...
  inline const_reverse_iterator	crend (void) const		{ return (rend()); }
    ...
    inline void			swap (string&& v)		{ memblock::swap (forward<string>(v)); }
    inline string&		assign (string&& v)		{ swap (forward<string>(v)); return (*this); }
    ...
    inline string&		operator= (string&& v)		{ return (assign (forward<string>(v))); }
