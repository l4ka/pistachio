...
#ifndef __ARCH__ALPHA__HWRPB_H_
#define __ARCH__ALPHA__HWRPB_H_
...
#define INIT_HWRPB        ((struct hwrpb *) HWRPB_ADDR)
#define INITIAL_VPT       (0x0000000200000000UL)

struct hwrpb {
	word_t	rpb_phys;		/*   0: HWRPB phys. address. */
	char		rpb_magic[8];		/*   8: "HWRPB" (in ASCII) */
	word_t	rpb_version;		/*  10 */
	word_t	rpb_size;		/*  18: HWRPB size in bytes */
	word_t	rpb_primary_cpu_id;	/*  20 */
	word_t	rpb_page_size;		/*  28: (8192) */
	u32_t	rpb_phys_addr_size;	/*  30: physical address size */
	u32_t	rpb_extended_va_size;	/*  34: extended VA size (4L) */
	word_t	rpb_max_asn;		/*  38:   (16) */
...
#define	SV_ST_MIATA_1_5		0x00004c00	/* Miata 1.5 */
	...
	word_t	rpb_variation;		/*  58 */

	char		rpb_revision[8];	/*  60; only first 4 valid */
	word_t	rpb_intr_freq;		/*  68; scaled by 4096 */
	word_t	rpb_cc_freq;		/*  70: cycle cntr frequency */
	word_t		rpb_vptb;		/*  78: */
	word_t	rpb_reserved_arch;	/*  80: */
	word_t		rpb_tbhint_off;		/*  88: */
	word_t	rpb_pcs_cnt;		/*  90: */
	word_t	rpb_pcs_size;		/*  98; pcs size in bytes */
	word_t		rpb_pcs_off;		/*  A0: offset to pcs info */ 
	word_t	rpb_ctb_cnt;		/*  A8: console terminal */
	word_t	rpb_ctb_size;		/*  B0: ctb size in bytes */
	word_t		rpb_ctb_off;		/*  B8: offset to ctb */
	word_t		rpb_crb_off;		/*  C0: offset to crb */
	word_t		rpb_memdat_off;		/*  C8: memory data offset */
	word_t		rpb_condat_off;		/*  D0: config data offset */
	word_t		rpb_fru_off;		/*  D8: FRU table offset */
	word_t	rpb_save_term;		/*  E0: terminal save */
	word_t	rpb_save_term_val;	/*  E8: */
	word_t	rpb_rest_term;		/*  F0: terminal restore */
	word_t	rpb_rest_term_val;	/*  F8: */
	word_t	rpb_restart;		/* 100: restart */
	word_t	rpb_restart_val;	/* 108: */
	word_t	rpb_reserve_os;		/* 110: */
	word_t	rpb_reserve_hw;		/* 118: */
	word_t	rpb_checksum;		/* 120: HWRPB checksum */
	word_t	rpb_rxrdy;		/* 128: receive ready */
	word_t	rpb_txrdy;		/* 130: transmit ready */
	word_t		rpb_dsrdb_off;		/* 138: HWRPB + DSRDB offset */
	word_t	rpb_tbhint[8];		/* 149: TB hint block */
};
 ...
 */
struct pcs {
    u8_t	pcs_hwpcb[128];		/*   0: PAL dependent */
...
#define	PCS_mbz	      0xffffffffff000000	/* 24:63 -- must be zero */
	...
	word_t	pcs_pal_memsize;	/*  88: PAL memory size */
	word_t	pcs_pal_scrsize;	/*  90: PAL scratch size */
	word_t		pcs_pal_memaddr;	/*  98: PAL memory addr */	
	word_t		pcs_pal_scraddr;	/*  A0: PAL scratch addr */
	struct {
		word_t
...
#define pcs_proc_cnt	pcs_pal_rev.proc_cnt
	...
	word_t	pcs_proc_type;		/*  B0: processor type */
...
#define	PCS_CPU_MINORTYPE(p) ((p)->pcs_proc_type >> 32)
	...
	word_t	pcs_proc_var;		/* B8: processor variation. */
...
#define	PCS_VAR_RESERVED	0xfffffffffffffff8	/* Reserved */
	...
	word_t		pcs_machcheck;		/*  D8: mach chk phys addr. */
	word_t	pcs_machcheck_len;	/*  E0: length in bytes */
	word_t		pcs_halt_pcbb;		/*  E8: phys addr of halt PCB */
	word_t		pcs_halt_pc;		/*  F0: halt PC */
	word_t	pcs_halt_ps;		/*  F8: halt PS */
	word_t	pcs_halt_r25;		/* 100: halt argument list */
	word_t	pcs_halt_r26;		/* 108: halt return addr list */
	word_t	pcs_halt_r27;		/* 110: halt procedure value */
...
#define	PCS_HALT_PTBR			8	/* 9-FF: reserved */
	word_t	pcs_halt_reason;	/* 118: */

	word_t	pcs_reserved_soft;	/* 120: preserved software */
...
#define	PALvar_OSF1	2
	word_t	pcs_palrevisions[16];	/* 1D0: PALcode revisions */

	word_t	pcs_reserved_arch[6];	/* 250: reserved arch */
};
 ...
 */
struct ctb {
	word_t	ctb_type;		/*   0: CTB type */
	word_t	ctb_unit;		/*   8: */
	word_t	ctb_reserved;		/*  16: */
	word_t	ctb_len;		/*  24: bytes of info */
	word_t	ctb_ipl;		/*  32: console ipl level */
	word_t		ctb_tintr_vec;		/*  40: transmit vec (0x800) */
	word_t		ctb_rintr_vec;		/*  48: receive vec (0x800) */
...
#define	CTB_NETWORK		0xC0		/* network device */
	word_t	ctb_term_type;		/*  56: terminal type */

	word_t	ctb_keybd_type;		/*  64: keyboard nationality */
	word_t		ctb_keybd_trans;	/*  72: trans. table addr */
	word_t		ctb_keybd_map;		/*  80: map table addr */
	word_t	ctb_keybd_state;	/*  88: keyboard flags */
	word_t	ctb_keybd_last;		/*  96: last key entered */
	word_t		ctb_font_us;		/* 104: US font table addr */
	word_t		ctb_font_mcs;		/* 112: MCS font table addr */
	word_t	ctb_font_width;		/* 120: font width, height */
	word_t	ctb_font_height;	/* 128:		in pixels */
	word_t	ctb_mon_width;		/* 136: monitor width, height */
	word_t	ctb_mon_height;		/* 144:		in pixels */
	word_t	ctb_dpi;		/* 152: monitor dots per inch */
	word_t	ctb_planes;		/* 160: # of planes */
	word_t	ctb_cur_width;		/* 168: cursor width, height */
	word_t	ctb_cur_height;		/* 176:		in pixels */
	word_t	ctb_head_cnt;		/* 184: # of heads */
	word_t	ctb_opwindow;		/* 192: opwindow on screen */
	word_t		ctb_head_offset;	/* 200: offset to head info */
	word_t		ctb_putchar;		/* 208: output char to TURBO */
	word_t	ctb_io_state;		/* 216: I/O flags */
	word_t	ctb_listen_state;	/* 224: listener flags */
	word_t		ctb_xaddr;		/* 232: extended info addr */
	word_t	ctb_turboslot;		/* 248: TURBOchannel slot # */
	word_t	ctb_server_off;		/* 256: offset to server info */
	word_t	ctb_line_off;		/* 264: line parameter offset */
	u8_t	ctb_csd;		/* 272: console specific data */
};

struct ctb_tt {
	word_t	ctb_type;		/*   0: CTB type */
	word_t	ctb_unit;		/*   8: console unit */
	word_t	ctb_reserved;		/*  16: reserved */
	word_t	ctb_length;		/*  24: length */
	word_t	ctb_csr;		/*  32: address */
	word_t	ctb_tivec;		/*  40: Tx intr vector */
	word_t	ctb_rivec;		/*  48: Rx intr vector */
	word_t	ctb_baud;		/*  56: baud rate */
	word_t	ctb_put_sts;		/*  64: PUTS status */
	word_t	ctb_get_sts;		/*  72: GETS status */
	word_t	ctb_reserved0;		/*  80: reserved */
};
...
struct crb {
	...
	word_t		 crb_p_dispatch;	/*   8: phys dispatch addr */
	struct crd	*crb_v_fixup;		/*  10: virtual fixup addr */
	word_t		 crb_p_fixup;		/*  18: phys fixup addr */
	word_t	 crb_map_cnt;		/*  20: phys/virt map entries */
	word_t	 crb_page_cnt;		/*  28: pages to be mapped */
    struct vf_map map[0];
};
 ...
 */
struct mddt {
	s64_t	 	mddt_cksum;		/*   0: 7-N checksum */
	word_t		mddt_physaddr;		/*   8: bank config addr
						 * IMPLEMENTATION SPECIFIC
						 */
	word_t	mddt_cluster_cnt;	/*  10: memory cluster count */
	struct mddt_cluster {
		word_t		mddt_pfn;	/*   0: starting PFN */
		word_t	mddt_pg_cnt;	/*   8: 8KB page count */
		word_t	mddt_pg_test;	/*  10: tested page count */
		word_t		mddt_v_bitaddr;	/*  18: bitmap virt addr */
		word_t		mddt_p_bitaddr;	/*  20: bitmap phys addr */
		s64_t		mddt_bit_cksum;	/*  28: bitmap checksum */
...
#define	MDDT_mbz	  0xfffffffffffffffc	/* 2:63 -- must be zero */
		s64_t		mddt_usage;	/*  30: bitmap permissions */
 ...
 */
struct dsrdb {
	s64_t		dsr_smm;		/*  0: SMM number */
	word_t	dsr_lurt_off;		/*  8: LURT table offset */
	word_t	dsr_sysname_off;	/* 16: offset to sysname */
};
